        ...
        <div class="filter-header-top">
          <div class="filter-header-left">
            <div class="filter-item">
              ...
              <el-input v-model="step" style="width: 80px;"/>
            </div>
          </div>
...
function bilibiliGoodsSearch(){
  ...
  console.time('bilibiliGoodsSearch');
  $.ajax({
    type: "POST",
    ...
    success: function (res) {
      console.timeEnd('bilibiliGoodsSearch'); 
      try {
          //在此运行代码
          if(!res) { return }
          if(res.code == 0){
            for(let i in res.data.data){
                let itemsId = res.data.data[i].detailDtoList.map((itema)=>{
                    return itema.skuId
                })
                res.data.data[i].itemsId = itemsId.sort().join(',')
                newAnalysis(res.data.data[i])
            }
          }
      }
      catch(err){
...
function newAnalysis(i){
    if(checkList[i.itemsId] && checkList[i.itemsId].price){
        console.log(i.c2cItemsName+' 现在的价格：'+i.showPrice+' 期望价格：'+checkList[i.itemsId].price)
        if(Number(i.showPrice) <= Number(checkList[i.itemsId].price)){ 
            console.log(`${i.c2cItemsName} 现在价格是${i.showPrice},开始查询商品详情`);  
            interval.log.unshift(`${i.c2cItemsName} 现在价格是${i.showPrice},开始查询商品详情`)
            const listStr = localStorage.getItem("LOCK_LIST") || ""
            if(listStr.indexOf(i.c2cItemsId) == -1){
                lockBill(i,i.itemsId) 
                console.log(`第一次：锁单`); 
                let newList =  listStr + i.c2cItemsId + '/'
                localStorage.setItem("LOCK_LIST",newList);
            }else{
                console.log(`重复：跳过`); 
            } 
        }
...
function lockBill(item,categoryId){
    console.log('开始锁单！！！！！！！！！')
    interval.log.unshift("开始锁单！！！！！！！")
    let data = {
      "source":"",
      ...
      "orderItems":{
          ...
          "subItemsList":item.detailDtoList.map((subItem)=>{
              let map = {
                  ...
                  "marketPrice":  String(subItem.marketPrice / 100),
    ...
    $.ajax({
      type: "POST",
      url: `http://111.229.88.32:7777/play_biligoods/api/magic-c/c2c/order/create?buvid=C8D14199-DF6A-662E-BA20-623D70997D4663385infoc&platform=h5&uid=2054000&channel=1&vtoken=&client-type=hyg`,
      timeout: 20000,
      headers : {
          "Content-Type": "application/json",
      ...
      success: function (res) {
        ...
        if(res.code == 0){
            ...
            console.log('------------------------------------------------------------------------')
            console.log('------------------------------------------------------------------------')
            let msg = `${item.c2cItemsName} 现在价格是${item.showPrice},锁单完成！！！！！！！！(web)`
            let searchUrl = `http://111.229.88.32:7777/play_biligoods/#/?id=${categoryId}`
            interval.log.unshift(msg)
            ...
            $.post(setOptions, (err, resp, res) => {
              $.get({url:`http://111.229.88.32:5701/send_private_msg?user_id=749975453&message=${msg}`,})  
              setOptions.json.message =  searchUrl
              $.post(setOptions,()=>{})
            });
        }else{
